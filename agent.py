...
from food_ordering.proposals import Add, ChangeQuantity, ClearDraft, Edit, MenuQuestion, NewOrder, Proposal, RemoveLine, Review, Submit, Summary, Unsupported
...
class FoodOrderAgent:
    ...
    def send(self, message: str) -> dict[str, Any]:
        ...
        try:
            ...
            if review_requested or any(isinstance(operation, Submit) for operation in proposal.operations):
                if self._status == "submitted":
                    response["message"] = self._receipt_message
                elif self._status in {"uncertain", "rejected"}:
                    response["message"] = "This submission cannot be repeated in this version. Please check with the restaurant."
                else:
                    payload = submission_payload(self._lines, self._menu)
                    if review_requested or changed or self._reviewed_revision != self._revision:
                        ...
                        if outcome.status == "submitted":
                            self._receipt_message = render_receipt(outcome.result)
                            response["message"] = self._receipt_message
                            ...
                            response["message"] = f"The restaurant rejected the order: {outcome.result.get('error', 'No explanation supplied')}. Your selections are preserved."
                        else:
